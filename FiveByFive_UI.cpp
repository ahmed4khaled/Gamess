...
#include <limits>
#include <ctime>
// Count 3-in-a-row windows for given matrix and symbol (used by AI)
int FiveByFive_UI::count_three_for_matrix(const std::vector<std::vector<char>>& mat, char symbol) const {
    ...
    for (int r = 0; r < R; ++r)
        for (int c = 0; c <= C - 3; ++c)
            if (mat[r][c] == symbol && mat[r][c+1] == symbol && mat[r][c+2] == symbol) ++cnt;
    // cols
    for (int c = 0; c < C; ++c)
        for (int r = 0; r <= R - 3; ++r)
            if (mat[r][c] == symbol && mat[r+1][c] == symbol && mat[r+2][c] == symbol) ++cnt;
    // diag "\"
    for (int r = 0; r <= R - 3; ++r)
        for (int c = 0; c <= C - 3; ++c)
            if (mat[r][c] == symbol && mat[r+1][c+1] == symbol && mat[r+2][c+2] == symbol) ++cnt;
    // diag "/"
    for (int r = 0; r <= R - 3; ++r)
        for (int c = 2; c < C; ++c)
            if (mat[r][c] == symbol && mat[r+1][c-1] == symbol && mat[r+2][c-2] == symbol) ++cnt;
...
void FiveByFive_UI::display_board_matrix(const std::vector<std::vector<char>>& matrix) const {
    // reuse base formatting or custom
...
Move<char>* FiveByFive_UI::get_move(Player<char>* player) {
    ...
    if (player->get_type() == PlayerType::HUMAN) {
        std::cout << player->get_name() << " (" << player->get_symbol() << ") - enter row and column (e.g. 1 3): ";
        int r = read_int_in_range(0,4);
        int c = read_int_in_range(0,4);
        return new Move<char>(r, c, player->get_symbol());
    } else {
      // inside get_move when player->get_type() == COMPUTER
...
for (auto pr : empties) {
    ...
    if (worstOpp == INT_MIN) worstOpp = 0;
    int distCenter = abs(r-2) + abs(c-2);
    int score = (myCount - worstOpp) * 10 - distCenter;
    mat[r][c] = '.';
    if (score > bestScore) { bestScore = score; bestMove = pr; }
}
